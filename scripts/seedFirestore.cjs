/*
Seed Firestore with sample data (CommonJS, for projects using "type": "module").

Usage:
- For emulator: set FIRESTORE_EMULATOR_HOST (e.g. localhost:8080) and run:
  FIRESTORE_EMULATOR_HOST=localhost:8080 node scripts/seedFirestore.cjs

- For real project: set GOOGLE_APPLICATION_CREDENTIALS to service account JSON path:
  GOOGLE_APPLICATION_CREDENTIALS=./serviceAccountKey.json node scripts/seedFirestore.cjs
*/
